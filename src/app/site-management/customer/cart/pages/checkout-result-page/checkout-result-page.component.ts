...
export class CheckoutResultPageComponent {
  ...
  private readonly checkoutResultStore = inject(CheckoutResultStore);
  ...
  protected readonly order = this.checkoutResultStore.order;
  protected readonly loading = this.checkoutResultStore.loading;
  private readonly routeError = signal<string | null>(null);
  protected readonly error = computed(() => this.routeError() ?? this.checkoutResultStore.error());
  ...
  constructor() {
    ...
    this.checkoutResultStore.load({
      orderId,
      pollForPaymentConfirmation: this.returnStatus === 'success',
    });
  }
